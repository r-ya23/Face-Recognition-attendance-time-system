                ...
                with open('Attendance.csv', 'a') as f:
                        if (name != 'unknown'):
                            ...
                            if buttonReply == QMessageBox.Yes:
                                ...
                                CheckInTime = self.TimeList1[-1]   #-1 gives the latest entry
                                CheckOutTime = self.TimeList2[-1]
                                self.ElapseHours = (CheckOutTime - CheckInTime)
                                self.hours_label.setText("{:.0f}".format(abs(self.ElapseHours.total_seconds() / 60 ** 2)) + 'h')
                                self.minutes_label.setText("{:.0f}".format(abs(self.ElapseHours.total_seconds() / 60) % 60) + 'm')
